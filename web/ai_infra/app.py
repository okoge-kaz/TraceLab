...
class SandboxPool:
    ...
    def prefetch(
        ...
        *,
        ...
        if touch_future is not None:
            future = self._refresh_checked_out_sandbox(
                key,
                touch_future=touch_future,
                template=template,
                sandbox_timeout=sandbox_timeout,
                allow_internet=allow_internet,
    ...
    def _refresh_checked_out_sandbox(
        self,
        ...
        *,
        ...
        try:
            touch_future.result().set_timeout(max(ttk_seconds, sandbox_timeout))
            return touch_future
        except Exception as exc:
            error = exc

        now = time.monotonic()
        old_record: SandboxRecord | None = None
        with self._lock:
            record = self._records.get(key)
            if record is None:
                future = self._executor.submit(
                ...
                record.active = 1
                self._records[key] = record
                status = "sandbox_timeout_refresh_failed_replaced"
            elif record.future is touch_future:
                old_record = record
                future = self._executor.submit(
                    runtime.create_sandbox,
                    template=template,
                ...
                replacement = SandboxRecord(future=future, expires_at=now + ttk_seconds)
                replacement.active = record.active
                self._records[key] = replacement
                status = "sandbox_timeout_refresh_failed_replaced"
            else:
                future = record.future
                record.expires_at = now + ttk_seconds
                status = "sandbox_timeout_refresh_already_replaced"

        if old_record is not None:
            self._retire(old_record)
    ...
    def replace_stale(
        ...
        *,
        ...
        """Replace an expired remote sandbox without dropping the session's warm-pool slot."""
        now = time.monotonic()
        old_future: Future | None = None
        with self._lock:
            record = self._records.get(key)
            if record is not None and record.future.done():
                try:
                    current = record.future.result()
                except Exception:
                    current = None
                if current is not None and current is not stale_sandbox:
                    record.expires_at = now + ttk_seconds
                    future = record.future
                    status = "stale_sandbox_already_replaced"
                else:
                    old_future = record.future
                    future = self._executor.submit(
                        runtime.create_sandbox,
                        template=template,
                        sandbox_timeout=sandbox_timeout,
                    ...
                    record.expires_at = now + ttk_seconds
                    status = "stale_sandbox_replacement_started"
            elif record is not None:
                future = record.future
                record.expires_at = now + ttk_seconds
                status = "stale_sandbox_replacement_pending"
            else:
                future = self._executor.submit(
                    runtime.create_sandbox,
                    template=template,
        ...
        if old_future is not None:
            self._kill_future_sandbox(old_future)

        logger(
            "e2b",
            {"status": status, "template": template, "session_id": key[0], "ttk_seconds": ttk_seconds},
        )
...
def response_locale(payload: dict[str, Any]) -> str:
    raw = str(payload.get("locale") or payload.get("language") or "").strip().lower()
    return "zh" if raw in {"zh", "zh-cn", "zh_cn", "cn", "chinese"} else "en"
...
def _chat_kwargs(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "response_locale": response_locale(payload),
    }
...
def _run_public_turn(pool: SandboxPool, payload: dict[str, Any], emit) -> dict[str, Any]:
    ...
    try:
        return runtime.run_chat_turn(
            ...
            sandbox_recovery=lambda stale: pool.replace_stale(
                key,
                stale_sandbox=stale,
                template=template,
                sandbox_timeout=sandbox_timeout,
...
def _llm_call_meta(value: dict[str, Any]) -> dict[str, Any]:
    perf = value.get("perf") if isinstance(value.get("perf"), dict) else {}
    usage = value.get("usage") if isinstance(value.get("usage"), dict) else {}
    meta = {
        "id": value.get("id"),
        "model": value.get("model"),
        "attempt": value.get("attempt"),
        "turn_elapsed_ms": value.get("turn_elapsed_ms"),
        "wall_ms": perf.get("wall_ms"),
        "latency_ms": perf.get("latency_ms"),
        "generation_time_ms": perf.get("generation_time_ms"),
        "completion_tokens": usage.get("completion_tokens") or perf.get("completion_tokens"),
        "provider_name": perf.get("provider_name"),
    }
...
def _rounds_from_events(events: list[tuple[str, Any]], *, log_content: bool) -> list[dict[str, Any]]:
    ...
    for label, value in events:
        if label in ("openrouter_retry", "generation_retry"):
            ...
            if pending_llm_calls:
                cur["llm_calls"] = pending_llm_calls
                pending_llm_calls = []
            if log_content:
            ...
            if entry is not None:
                ...
                if value.get("duration_ms") is not None:
                    entry["duration_ms"] = value.get("duration_ms")
                if value.get("turn_elapsed_ms") is not None:
                    entry["turn_elapsed_ms"] = value.get("turn_elapsed_ms")
    ...
    if pending_llm_calls:
        rounds.append({"round": "final", "llm_calls": pending_llm_calls})
    return rounds
...
# ---- app ----------------------------------------------------------------------------------
app = FastAPI(title="SyFI TraceLab Ask API")
    ...
    async def run_turn(payload: dict[str, Any]) -> None:
        ...
        def turn_emit(label: str, value: Any) -> None:
            ...
            if isinstance(value, dict):
                ...
                event_value.setdefault("turn_elapsed_ms", round((time.monotonic() - started) * 1000))
                event_value.setdefault("event_ts", datetime.now(timezone.utc).isoformat())
            events.append((label, event_value))
            emit(label, event_value)
