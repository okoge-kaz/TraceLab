...
DEFAULT_PRICING_FILE = Path(__file__).resolve().parents[2] / "artifacts" / "utils" / "pricing.json"
...
def pricing_context(pricing_file: Path = DEFAULT_PRICING_FILE) -> str:
    """Render the single-source price table as prompt context for trace cost questions."""
    ...
    return f"""Pricing source: `artifacts/utils/pricing.json`
Reviewed as of: `{table.get("as_of", "unknown")}`
...
excluded instead of estimating their cost.
...
`reasoning_output_tokens` are already included in `output_tokens`; report `reasoning_cost` only as
a subset of output cost, not an additional charge.
...
def is_stale_sandbox_error(exc: BaseException) -> bool:
    """Return true for E2B errors caused by a sandbox that no longer exists."""
    text = f"{type(exc).__name__}: {exc}".lower()
    return "sandbox was not found" in text
...
def _generation_stats_url(chat_url: str, generation_id: str) -> str:
    base = chat_url.rstrip("/")
    if base.endswith("/chat/completions"):
        base = base[: -len("/chat/completions")]
    query = urllib.parse.urlencode({"id": generation_id})
    return f"{base}/generation?{query}"


def _fetch_generation_stats(
    *,
    api_key: str | None,
    chat_url: str,
    generation_id: str | None,
) -> dict[str, Any] | None:
    if not generation_id or not _is_openrouter_url(chat_url):
        return None
    request = urllib.request.Request(_generation_stats_url(chat_url, generation_id), method="GET")
    if api_key:
    ...
    for attempt, delay in enumerate((0.0, 0.5, 1.0, 2.0), start=1):
        ...
        try:
            ...
            break
        except Exception as exc:  # noqa: BLE001 — generation stats are best-effort instrumentation
            last_error = f"{type(exc).__name__}: {exc}"
            if attempt == 4:
                return {"error": last_error}
    else:
        return {"error": last_error or "generation stats unavailable"}
...
def _derive_call_perf(
    *,
    ...
    completion_tokens = completion_tokens or _coerce_float(
        stats.get("native_tokens_completion") or stats.get("tokens_completion")
    )
    latency_ms = _coerce_float(stats.get("latency"))
    generation_time_ms = _coerce_float(stats.get("generation_time"))
    total_cost = _coerce_float(stats.get("total_cost") or stats.get("usage"))
    if total_cost is None and usage:
        total_cost = _coerce_float(usage.get("cost"))

    # OpenRouter generation stats expose total latency and generation time, but non-streaming
    # completions do not expose true client-observed TTFT. Keep the estimate explicitly named.
    ttft_estimated_ms = None
    if latency_ms is not None and generation_time_ms is not None:
        ttft_estimated_ms = max(0.0, latency_ms - generation_time_ms)

    tpot_estimated_ms = None
    if generation_time_ms is not None and completion_tokens and completion_tokens > 0:
        tpot_estimated_ms = generation_time_ms / completion_tokens

    observed_completion_tokens_per_second = None
    if completion_tokens and completion_tokens > 0 and wall_ms > 0:
        observed_completion_tokens_per_second = completion_tokens / (wall_ms / 1000.0)
    ...
    return {
        ...
        "ttft_ms": None,
        "ttft_estimated_ms": ttft_estimated_ms,
        "tpot_ms": None,
        "tpot_estimated_ms": tpot_estimated_ms,
        "completion_tokens": int(completion_tokens) if completion_tokens is not None else None,
        "observed_completion_tokens_per_second": (
            round(observed_completion_tokens_per_second, 6)
            if observed_completion_tokens_per_second is not None
            else None
        ),
        "cost": total_cost,
    ...
    *,
    ...
    for attempt in range(max_retries + 1):
        ...
        if is_openrouter:
            ...
            headers["X-Title"] = "SyFI TraceLab QA"
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=120) as response:
                ...
                generation_stats = _fetch_generation_stats(
                    api_key=api_key,
                    chat_url=chat_url,
                    generation_id=generation_id,
                )
                perf = _derive_call_perf(
                    wall_ms=wall_ms,
                    usage=usage,
                    generation_stats=generation_stats,
                )
                usage_entry = dict(usage or {})
                if perf.get("cost") is not None and usage_entry.get("cost") is None:
                    usage_entry["cost"] = perf["cost"]
                if perf.get("completion_tokens") is not None and usage_entry.get("completion_tokens") is None:
                    usage_entry["completion_tokens"] = perf["completion_tokens"]
                usage_entry.update(
                    {
                        "id": generation_id,
...
def system_prompt(
    ...
    *,
    ...
    if response_locale == "zh":
        language = (
            "Response language: Answer user-facing prose in Simplified Chinese. Keep code, SQL/Python "
            "identifiers, database column names, provider/model names, and exact metric names in English "
            "when necessary. Use Chinese chart titles, axis labels, and markdown headings when you create "
            "user-facing plots or explanations."
        )
    else:
        language = (
            "Response language: Answer user-facing prose in English. Keep code comments and generated "
            "chart labels in English unless the user explicitly asks otherwise."
        )
    return f"{rendered}\n\n{language}"
...
class E2BPythonExecutor(ToolExecutor):
    ...
    def __init__(
        ...
        *,
        ...
        sandbox_recovery: Callable[[Any], Any] | None = None,
        ...
        self.sandbox_recovery = sandbox_recovery
        self.kill_sandbox = kill_sandbox
        self.sandbox = None
        self.sandbox_prepared = False

    def _replace_stale_sandbox(self, logger, exc: BaseException):
        started = time.perf_counter()
        stale_sandbox = self.sandbox
        _event(
            logger,
            "e2b",
            {
        ...
        if self.sandbox_recovery is not None:
            self.sandbox = self.sandbox_recovery(stale_sandbox)
        else:
            if stale_sandbox is not None:
                try:
                    stale_sandbox.kill()
                except Exception:
                    pass
            _event(logger, "e2b", {"status": "creating_sandbox", "template": self.template, "recovered": True})
    ...
    def ensure_sandbox(self, logger):
        ...
        if self.sandbox is None:
            ...
            _event(
                ...
                "e2b",
                {
                    ...
                    "template": self.template,
                    "sandbox_id": getattr(self.sandbox, "sandbox_id", None),
                    "duration_ms": round((time.perf_counter() - ready_started) * 1000),
                },
            )
        if not self.sandbox_prepared:
            ...
            try:
                ...
                if not is_stale_sandbox_error(exc):
                    raise
                self._replace_stale_sandbox(logger, exc)
                prepare_started = time.perf_counter()
                reset_output_dir(self.sandbox)
            ...
            _event(
                ...
                "e2b",
                {
                    ...
                    "template": self.template,
                    "sandbox_id": getattr(self.sandbox, "sandbox_id", None),
                    "duration_ms": round((time.perf_counter() - prepare_started) * 1000),
                },
            )
        return self.sandbox
