...
## Trace Semantics And Derived Metrics

The trace records model steps, tool calls, and timing observations. A round is one model step, not
necessarily a full user request: a single human message can trigger an initial model response plus
several continuation rounds driven by tool results. Child tables such as `tool_calls` and
`timing_events` can contain many rows per round, so keep original round counts separate from joined
child-row counts when reporting denominators.

Some useful metrics are derived from the event stream rather than stored as columns. For derived
timing metrics, preserve trace order (`round_pk`/ingestion order) and same-session state; do not
assume that grouping into user requests and measuring idle human gaps are interchangeable. Request
duration metrics describe the active span from a request-start user message through the request's
model/tool continuation rounds. Human-wait metrics describe idle gaps between model output already
observed in the trace and a later user message that starts a new request. If a user names a canonical
SYFI metric, use the metric definition in this prompt rather than inventing a near-equivalent proxy.
...
- Human input wait: within one session, the idle gap from model output already observed in trace order to the next request-start user message. It is a human-in-the-loop idle-time metric, not the same aggregation as request end-to-end response time.
...
For cost questions, first compute token totals from the DuckDB trace with `run_python`, then apply the
pricing table and formulas below. Do not answer cost questions from the prompt alone: prices come
from this prompt context, but token counts and models must come from the database.

Always report cost coverage: include the number of priced rounds and unpriced rounds. If any rounds
are unpriced, explicitly say they were excluded from dollar totals. Never infer or guess prices for
unknown provider/model pairs.
