...
const chartText = (label: string) => browserCopy().charts.distributions.labelMap[label] ?? label;
...
function fmtDurMs(v: number): string {
  ...
  if (v >= 60000) {
    ...
    if (Math.abs(min - r) < 0.02 * r) return zh ? `${r} 分钟` : `${r} min`;
  }
  return zh ? `${+(v / 1000).toFixed(v < 10000 ? 1 : 0)} 秒` : `${+(v / 1000).toFixed(v < 10000 ? 1 : 0)} s`;
}
...
function histogram(s: HistogramSpec, c: ThemeColors, name: string): echarts.EChartsCoreOption {
  ...
  if (s.series) {
    return {
      ...
      toolbox: saveAsImage(c, name),
      tooltip: { ...(baseOption(c).tooltip as object), trigger: 'axis' },
      xAxis: { type: s.xLog ? 'log' : 'value', ...axis(c, chartText(s.xLabel)), axisLabel: { color: c.muted, fontFamily: c.font, fontSize: c.fsTick, formatter: compactNum } },
      yAxis: { type: 'value', ...axis(c) },
      series: s.series.map((ser) => ({ name: chartText(ser.name), type: 'line', showSymbol: false, smooth: true, areaStyle: { opacity: 0.07 }, data: ser.points })),
    };
  }
  ...
  return {
    ...
    xAxis: { type: 'category', data: (s.bins ?? []).map((b) => chartText(b.label)), ...axis(c, chartText(s.xLabel)) },
    yAxis: { type: 'value', ...axis(c) },
...
function boxplot(s: BoxplotSpec, c: ThemeColors, name: string): echarts.EChartsCoreOption {
  return {
    ...
    xAxis: { type: 'category', data: s.groups.map((g) => chartText(g.name)), ...axis(c, ''), axisLabel: { color: c.muted, fontFamily: c.font, fontSize: c.fsTick, interval: 0, lineHeight: c.fsTick + 3, formatter: (v: string) => wrapLabel(v) } },
...
function scatter(s: ScatterSpec, c: ThemeColors, name: string): echarts.EChartsCoreOption {
  ...
  return {
    ...
    tooltip: {
      ...
      trigger: 'item',
      formatter: (p: any) =>
        `${p.marker} ${p.seriesName}<br>${browserCopy().charts.distributions.prefix} ${compactNum(p.value[0])} · ${browserCopy().charts.distributions.append} ${compactNum(p.value[1])}`,
    },
    xAxis: { type: s.xLog ? 'log' : 'value', ...axis(c, chartText(s.xLabel)), axisLabel: { color: c.muted, fontFamily: c.font, fontSize: c.fsTick, formatter: compactNum } },
...
function barh(s: BarhSpec, c: ThemeColors, name: string): echarts.EChartsCoreOption {
  ...
  return {
    ...
    xAxis: { type: 'value', ...axis(c, chartText(s.xLabel)) },
    yAxis: { type: 'category', data: items.map((i) => chartText(i.name)), ...axis(c, '') },
    series: [
      {
        type: 'bar',
