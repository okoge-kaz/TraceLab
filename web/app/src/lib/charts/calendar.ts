  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (p: any) => {
        ...
        return `<b>${d.day}</b><br/>${d.rounds} ${t.steps}<br/>${compact(d.inputTokens)} ${t.in} · ${compact(
          d.outputTokens,
        )} ${t.out}<br/>${fmtUsd(d.costUsd)}`;
      },
    },
    ...
    calendar: {
      ...
      dayLabel: { color: c.muted, fontFamily: c.font, fontSize: c.fsTick, firstDay: 1, nameMap: t.calendarDayMap },
