...
export function initContributeDialog(): void {
  ...
  const text = browserCopy().contribute;
  ...
  function updateProgress(p: UploadProgress) {
    ...
    if (p.etaSeconds != null && p.etaSeconds > 0.5) parts.push(`~${fmtSeconds(p.etaSeconds)} ${text.etaLeft}`);
  ...
  async function run(file: File) {
    setResult('processing', text.uploadTitle, text.uploadBody);
      ...
      const r = await contribute(file, {
        ...
        onPhase: (phase) => {
          if (phase === 'validating') {
            ...
            showProgress(false);
            setResult('processing', text.validatingTitle, text.validatingBody);
          }
        },
      });
      showProgress(false);
      if (r.duplicate || (r.accepted === 0 && r.skipped_sessions > 0)) {
        setResult('ok', text.duplicateTitle, text.duplicateBody);
      } else if (r.accepted === 0) {
        setResult('ok', text.nothingTitle, text.nothingBody);
      } else {
        const sess = r.new_sessions === 1 ? text.oneSession : template(text.manySessions, { count: r.new_sessions });
        setResult('ok', text.acceptedTitle,
          template(text.acceptedBody, { sessions: sess, rows: intComma(r.rows_added) }));
      }
      markAnalyzedTraceContributed(file);
      window.dispatchEvent(new CustomEvent('contrib:success', { detail: { file } }));
    } catch (err) {
      showProgress(false);
      setResult('error', text.errorTitle,
        err instanceof Error ? err.message : text.errorBody);
    }
  }
